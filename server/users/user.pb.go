...
const file_users_user_proto_rawDesc = "" +
	"\n" +
	"\x10users/user.proto\x12\x05users\"\a\n" +
	"\x05Empty\"#\n" +
	"\aMessage\x12\x18\n" +
	...
	"\x04code\x18\x02 \x01(\x05R\x04code2\x94\x02\n" +
	"\vUserService\x122\n" +
	"\aGetUser\x12\x12.users.UserRequest\x1a\x13.users.UserResponse\x126\n" +
	"\n" +
	"CreateUser\x12\x11.users.UserCreate\x1a\x15.users.CreationResult\x12.\n" +
	"\fClientStream\x12\x0e.users.Message\x1a\f.users.Empty(\x01\x12.\n" +
	"\fServerStream\x12\f.users.Empty\x1a\x0e.users.Message0\x01\x129\n" +
	"\x13BidirectionalStream\x12\x0e.users.Message\x1a\x0e.users.Message(\x010\x01B\x1aZ\x18github.com/grpc-go;usersb\x06proto3"
...
var file_users_user_proto_goTypes = []any{
	...
	(*Message)(nil),        // 1: users.Message
	(*User)(nil),           // 2: users.User
	(*UserCreate)(nil),     // 3: users.UserCreate
	(*UserRequest)(nil),    // 4: users.UserRequest
	(*UserResponse)(nil),   // 5: users.UserResponse
	(*CreationResult)(nil), // 6: users.CreationResult
	nil,                    // 7: users.User.DeatailsEntry
}
var file_users_user_proto_depIdxs = []int32{
	7, // 0: users.User.deatails:type_name -> users.User.DeatailsEntry
	4, // 1: users.UserService.GetUser:input_type -> users.UserRequest
	3, // 2: users.UserService.CreateUser:input_type -> users.UserCreate
	1, // 3: users.UserService.ClientStream:input_type -> users.Message
	0, // 4: users.UserService.ServerStream:input_type -> users.Empty
	1, // 5: users.UserService.BidirectionalStream:input_type -> users.Message
	5, // 6: users.UserService.GetUser:output_type -> users.UserResponse
	6, // 7: users.UserService.CreateUser:output_type -> users.CreationResult
	0, // 8: users.UserService.ClientStream:output_type -> users.Empty
	1, // 9: users.UserService.ServerStream:output_type -> users.Message
	1, // 10: users.UserService.BidirectionalStream:output_type -> users.Message
